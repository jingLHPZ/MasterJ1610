 ...
 * 和字符串有关的小工具.
 * @author pillar liu
 * @version $Id: StringUtils.java, v 0.1 2015-9-7 下午3:26:14 pillar liu $
 */
public class StringUtil {

    private static final OutputStream DUMMY_OUTPUT_STREAM = new ByteArrayOutputStream(0);
    private static final String       SYSTEM_CHARSET      = new OutputStreamWriter(
                                                              DUMMY_OUTPUT_STREAM).getEncoding();
     ...
     */
    public static List<String> splitStr(String sStr, String sSplitter) {
     ...
     * @return 被替换后的字符串，如果原始字符串为<code>null</code>，则返回<code>null</code>
     */
    public static String replaceString(String str, String searchChar, String replaceChar) {
        if (str == null) {
            return null;
        }
    ...
    public static String getPerMoney(String payMoney){
    	String str ="";
    	if (payMoney.length()>5){
	    	String tempStr =  payMoney.substring(0, payMoney.length()-3);
	    	String tempStr1 =  payMoney.substring(payMoney.length()-3, payMoney.length());
	    	int i = tempStr.length()/3;
	    	...
	    	if(i>0){
	    		for(int j=0;j<=i;j++){
	    			if (tempStr.length()<=j*3+3)
	    				strBuf.append(str.substring(j*3, tempStr.length()));
	    			else
	    				strBuf.append(str.substring(j*3, j*3+3)).append(",");
	    		}
	    	}else{
	    		strBuf.append("0");
	    	}
	    	...
	    	return strBuf.reverse().toString()+tempStr1;
    	}else{
    		return payMoney;
    	}
    }
    
    protected boolean getPay(String payno){
    	List<String> payList = new ArrayList<String>();
    	payList.add("b");
    	payList.add("d");
    	boolean result = false;
    	for(String payNo:payList){
			if(payno.equals(payNo)){
				result = true;
				return result;
			}
		}
    ...
    public static void main(String []ars){
    	Money s = new Money();
		//s.getRefundRmbTotals().setCurrency(Currency.getInstance(CurrencyEnum.CNY.getCode()));
    	
    		Money amount = new Money("0", Currency.getInstance("CNY"));
    		amount.setCent(Long.parseLong("10"));
