...
import com.sxt.enums.CurrencyEnum;
 ...
 * </ul>
 * @author Eric.fu
 * @version $Id: Money.java, v 0.3 2015-09-6 上午10:30:47 pillar liu Exp $
 */
public class Money implements Serializable, Comparable<Object>, Cloneable {
    /**
    ...
    public Money(BigDecimal amount, Currency currency, RoundingMode roundingMode) {
        ...
        this.setAmountWithRoundingMode(amount, roundingMode);
    }
     ...
     */
    public void setAmountFromString(String amount) {
        this.setAmountWithRoundingMode(new BigDecimal(amount), DEFAULT_ROUNDING_MODE);
    }
    ...
    public void setAmount(BigDecimal amount) {
        this.setAmountWithRoundingMode(amount, DEFAULT_ROUNDING_MODE);
    }
     ...
     * @return
     */
    public void setAmountFromString(String amount, RoundingMode roundingMode) {
        this.setAmountWithRoundingMode(new BigDecimal(amount), roundingMode);
    }
     ...
     * @return
     */
    public void setAmountWithRoundingMode(BigDecimal amount, RoundingMode roundingMode) {
        if (amount != null) {
    ...
    */
    ...
    public CurrencyEnum getCurrencyEnum(){
        return CurrencyEnum.getByCurrency(currency);
    }
     ...
     * 
     * <p>
     * venus反序列化时，按次序（1）调用构造器、（2）setAmount()、（3）setCent()、（4）setCurrency(),<br/>
     * 其中（3）中再调用（2）时，将使用初始币种获取元/分比例来更新amount值，此处将造成数据失真。
     * 
     * 
     * @deprecated
     */
    public void setCurrency(Currency currency) {
        String oldCode = this.currency.getCurrencyCode();
        
        boolean needResetCent = false;
        long currentCent = getCent();
        if(!oldCode.equalsIgnoreCase(currency.getCurrencyCode())){
            // 币种不同，需比较两种货币变化之间的数值变化.
            BigDecimal oldFactor = getCentFactor();
            BigDecimal newFactor = new BigDecimal(10).pow(currency.getDefaultFractionDigits());
            if(oldFactor.compareTo(newFactor)!=0){
                //比例不同，需转换。正常来讲，原币种为CNY，且比例为100
                //新币种为真实币种，需恢复到正确的amount。
                needResetCent = true;
            }
        }
    ...
    public Money multiplyBy(BigDecimal val, RoundingMode roundingMode) {
        this.setAmountWithRoundingMode(amount.multiply(val), roundingMode);
    ...
    public Money divideBy(BigDecimal val, RoundingMode roundingMode) {
        this.setAmountWithRoundingMode(amount.divide(val), roundingMode);
    ...
    protected Money newMoneyWithSameCurrency(BigDecimal amount, RoundingMode roundingMode) {
        ...
        money.setAmountWithRoundingMode(amount, roundingMode);
